...
class KmosDevDeployStage extends PullScriptStage {

    def config
    def backup
    KmosDevDeployStage(project, stageName, backup) {
    ...
    def run() {
        ...
        this.script.node('jenkins-slave-k8s') {
            ...
            this.script.echo "KmosDevDeployStage serverEnv:${serverEnv}"

            this.script.withEnv(serverEnv) {
                if (this.backup){
                    try {
                      this.script.echo "KmosDevDeployStage backup deploy.yaml"
                ...
                this.script.sh "sed -i 's/value: dev/value: kmos_dev/g' ${deploy_script_dir}/${sub_dir}/k8s/dev/deploy.yml"
                this.script.sh "sed -i '\$a\\      imagePullSecrets:\\n        - name: hc-habor-secret\\n' ${deploy_script_dir}/${sub_dir}/k8s/dev/deploy.yml"
                this.script.sh "sed  -i 's/192.168.100.36:1179\\/xiaoke/registry.cn-beijing.aliyuncs.com\\/kaikeba/g' ${deploy_script_dir}/${sub_dir}/k8s/dev/deploy.yml"
                def yamlfile = this.script.sh(returnStdout: true, script:"cat ${this.script.env.WORKSPACE}/${deploy_script_dir}/${sub_dir}/k8s/dev/deploy.yml")
                this.script.sh "echo '----------------------------'"
                def output = this.addStartTime(yamlfile)
                this.script.sh("echo \"\"\"${output}\"\"\">${deploy_script_dir}/${sub_dir}/k8s/dev/deploy.yml")
