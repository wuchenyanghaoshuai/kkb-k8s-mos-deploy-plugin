...
class QADeployStage extends PullScriptStage {


    def config
    def backup
    QADeployStage(project, stageName, backup) {
    ...
    def run() {
        ...
        this.script.node('jenkins-slave-k8s') {
            ...
            serverEnv.add("K8S_DEPLOY_YML=" + "${deploy_script_dir}/${sub_dir}" + "/k8s/test/deploy.yml")
            serverEnv.add("K8S_SVC_YML=" + "${deploy_script_dir}/${sub_dir}" + "/k8s/test/svc.yml")
            ...
            this.script.withEnv(serverEnv) {
                ...
                def yamlfile = this.script.sh(returnStdout: true, script:"cat ${this.script.env.WORKSPACE}/${deploy_script_dir}/${sub_dir}/k8s/test/deploy.yml")
                this.script.sh "echo '----------------------------'"
                def output = this.addStartTime(yamlfile)
                this.script.sh("echo \"\"\"${output}\"\"\">${deploy_script_dir}/${sub_dir}/k8s/test/deploy.yml")
